...
public class BoardState {
	...
	public BoardState(String str) {
		...
		if (rows.length == 4) {
			for (int i = 0; i < 4; ++i) {
				...
				for (int j = 0; j < 4; ++j) {
					myTiles[4 * i + j] = new Tile(Integer.parseInt(cells[j + 1]));
				}
			}
		} else {
			String row = rows[0];
			String[] cells = row.split(Pattern.quote("|"));
			assert cells.length == 17;
			for (int j = 0; j < 16; ++j) {
				myTiles[j] = new Tile(Integer.parseInt(cells[j + 1]));
			}
		}
	}
	...
	public Triplet<BoardState, Integer, Boolean> evaluateMove(Move move) {
		...
		deepcopyTiles(myTiles, oldState);
		...
		// Revert state - shallow copy is OK;
		System.arraycopy(oldState, 0, myTiles, 0, 16);
		return output;
	}
	...
	public Triplet<BoardState, Integer, Boolean> left() {
		...
		BoardState afterState = new BoardState(this);
		if (needAddTile) {
			addTile();
		}
	...
	private static void deepcopyTiles(Tile[] src, Tile[] dst) {
		assert src.length == dst.length;
		for (int i = 0; i < src.length; ++i) {
			dst[i] = new Tile(src[i]);
		}
	}
	...
	public boolean equals(Object o) {
		...
		if (b.myTiles.length != this.myTiles.length)
			return false;
		for (int x = 0; x < 4; x++)
			for (int y = 0; y < 4; y++)
	...
	public int hashCode() {
		...
		for (int x = 0; x < 4; x++)
			for (int y = 0; y < 4; y++)
				hash = 31 * hash + (this.tileAt(x, y) == null ? 0 : this.tileAt(x, y).hashCode());
		return hash;
	}
}